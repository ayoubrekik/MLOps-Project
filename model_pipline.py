...
def encode_categorical_features(data):
    ...
    print(f"State categories: {le_state.classes_}")
...
def train_model(X_train_st, X_test_st, y_train):
    mlp = MLPClassifier()
    mlp.fit(X_train_st, y_train)
    y_pred_neural = mlp.predict(X_test_st)
    return y_pred_neural


def evaluate_model(y_test, y_pred):
    cm_nn = confusion_matrix(y_test, y_pred)
    disp = ConfusionMatrixDisplay(cm_nn, display_labels=["Stay", "Churn"])
...
def improve_model(X_train_st, X_test_st, y_train):
    param_grid = {
